...
The `portfolio-reset` Edge Function securely resets portfolios, while
`account-actions` permanently deletes accounts. Each validates the signed-in
user's access token before using the server-only service-role key supplied
automatically by Supabase. Never put that service-role key in frontend code.
...
npx supabase functions deploy portfolio-reset --no-verify-jwt --use-api
